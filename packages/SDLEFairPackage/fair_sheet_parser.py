...
class FairSheetParser:
    ...
    Attributes:
        ...
        __include_valuetype_graphviz_edges (bool): Flag that determines whether or not to include valuetype and unit edges in Graphviz graph.
    ...
    def __init__(self, ontology_sheet_folder, include_valuetype_graphviz_edges, rdflib_graph, graphviz_graph):
        """
        ...
        Args:
            ontology_sheet_folder (str): The folder containing ontology CSV files.
            include_valuetype_graphviz_edges (bool): Flag to include valuetype and unit edges in the Graphviz png.
            rdflib_graph (rdflib.Graph): An empty rdflib graph
            graphviz_graph (graphviz.Digraph): An empty graphviz graph
        """
        ontology_sheet_folder = Path(ontology_sheet_folder)

        ontology_info_path = next(iter(ontology_sheet_folder.glob("*OntologyInfo.csv")))
        obj_property_path = next(iter(ontology_sheet_folder.glob("*RelationshipDefinitions.csv")), None)
        data_property_path = next(iter(ontology_sheet_folder.glob("*ValueTypeDefinitions.csv")), None)
        namespace_path = next(iter(ontology_sheet_folder.glob("*NameSpace.csv")))
        entity_path = next(iter(ontology_sheet_folder.glob("*VariableDefinitions.csv")), None)
        ...
        self.__obj_property_uris = self.parse_object_properties()
        self.__data_property_uris = self.parse_data_properties()
        ...
        with open(self.__entity_path, newline='') as entity_file:
            ...
            def dfs(entity, entities_to_process):
                ...
                if entity['parent'] != "" and (entities_to_process.__contains__(entity['parent'])):
                    ...
                    if self.__include_valuetype_graphviz_edges and entity["unit"] != "":
